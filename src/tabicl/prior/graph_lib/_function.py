...
class RandomFunction(RandomTensorTransformer):
    ...
    """
    def __init__(self, context: Context, in_features: int, out_features: int, physics_allowed: bool = True,
                 force_physics: bool = False):
        """
        :param context: Context.
        :param in_features: Input dimensionality.
        :param out_features: Output dimensionality.
        :param physics_allowed: If False, excludes physics-informed function types from the random
            choice. Used to keep target/y nodes free of physics functions, since those currently
            broadcast a single scalar to out_features via a rank-1 linear map, which would make y
            trivially proportional to that scalar rather than a genuinely learnable function of x.
        :param force_physics: If True (and physics_allowed is also True), restricts the random choice
            to ONLY physics-informed function types. Used to bias specific nodes -- e.g. ancestors of
            y in the graph -- towards actually using a physics formula, instead of leaving it to chance
            whether physics ends up on the causal path to y at all.
        """
    ...
    def _fit(self, x: torch.Tensor):
        ...
        if not self.physics_allowed:
            fct_types = [t for t in fct_types if t not in PHYSICS_FCT_TYPES]
            if len(fct_types) == 0:  # fallback in case config.fct_types was e.g. 'physics' only
                fct_types = [RandomLinearFunction]
        elif self.force_physics:
            physics_only = [t for t in fct_types if t in PHYSICS_FCT_TYPES]
            if len(physics_only) > 0:  # otherwise config.fct_types has no physics types at all; ignore the force
                fct_types = physics_only

        fct_type = self.sampler.choice("random_function_fct_type", fct_types)
        self.fct_ = fct_type(self.context, self.in_features, self.out_features, physics_allowed=self.physics_allowed)
...
class RandomToleranceStackupFunction(RandomFunction):
    ...
    def _fit(self, x: torch.Tensor):
        ...
        if max_dims <= 2:
            n_dims = max_dims  # not enough columns for a real random range
        else:
...
# Set of physics function classes, used by RandomFunction/CheapRandomFunction to filter them
# out for target (y) nodes via the physics_allowed flag. Defined here (after the classes exist)
# and referenced only inside method bodies elsewhere in this file, so forward-reference order is fine.
PHYSICS_FCT_TYPES = {
    RandomArrheniusFunction,
    RandomArchardWearFunction,
    RandomHollomonHardeningFunction,
