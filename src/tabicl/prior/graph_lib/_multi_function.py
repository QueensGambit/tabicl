...
class RandomMultiFunction(RandomTransformer):
    ...
    """
    def __init__(self, context: Context, out_features: int, physics_allowed: bool = True,
    ...
    def _fit(self, x: List[torch.Tensor]):
        ...
        self.func_ = func_type(self.context, self.out_features, physics_allowed=self.physics_allowed,
                                force_physics=self.force_physics)
...
class RandomConcatMultiFunction(RandomMultiFunction):
    def _fit(self, x: List[torch.Tensor]):
        ...
        self.func_ = RandomFunction(self.context, in_features, self.out_features,
                                     physics_allowed=self.physics_allowed, force_physics=self.force_physics)
...
class RandomAggregationMultiFunction(RandomMultiFunction):
    def _fit(self, x: List[torch.Tensor]):
        ...
        self.funcs_ = [
            RandomFunction(self.context, tens.shape[-1], self.out_features,
                            physics_allowed=self.physics_allowed, force_physics=self.force_physics)
            for tens in x
        ]
